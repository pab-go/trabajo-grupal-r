...
bigotes_rural <-ggplot(votos_por_zona2) +
 ...
 graficos_unids <- barras_rural+bigotes_rural+ 
...
-   ¿Cómo calibrar el error de las encuestas (recordemos que las encuestas son de intención de voto a nivel nacional)?
  ...
  "many-to-many") |> select(-c(participacion_1,participacion_2,votos_blancos,votos_nulos,numero_mesas,zona,size,turnout,partidos))
...
¿Hay medios que tienden a beneficiar más a un partido que a otro?

```{r}

elecciones_encuestas_filt2 <- elecciones_encuestas |> 
  mutate(
    field_date_to = as_date(field_date_to),  # Asegúrate de que las fechas estén en formato Date
    date_elec = as_date(date_elec)
  ) |> 
  group_by(date_elec, media, partido) |>  # Agrupamos por elección, medio y partido
  filter(field_date_to == max(field_date_to[field_date_to <= date_elec])) |>  # Filtrar la fecha más cercana anterior a la elección
  ungroup()

# Verificamos el resultado
print(elecciones_encuestas_filt2)


# Paso 1: Calcular los votos estimados y el error absoluto
errores <- elecciones_encuestas_filt2 |> 
  group_by(date_elec) |> # Agrupar por elección 
  mutate(
    total_votos_eleccion = sum(votos, na.rm = TRUE)    # Calcular total de votos por elección
  ) |> 
  ungroup() |> # Salir de la agrupación
  mutate(
    votos_estimados = puntuacion / 100 * votos_candidaturas,  # Calcular votos estimados
    error = votos_estimados - votos ,                     # Calcular el error 
    error_relativo = abs(error/votos_candidaturas)
  )


# Paso 2: Agrupar por medio y partido para calcular el error medio
media_errores <- errores |> 
  group_by(media, partido) |> 
  summarise(
    error_medio = mean(error, na.rm = TRUE),          # Promedio del error absoluto
    error_mediano = median(error, na.rm = TRUE),      # Mediana del error absoluto
    n_encuestas = n_distinct(date_elec),
    relativo_medio = mean(error_relativo),  
    .groups = "drop"
  )
...
# Paso 3: Visualización del error medio por medio y partido
ggplot(media_errores, aes(x = partido, y = error_medio, fill = media)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Error medio por medio y partido",
    x = "Partido",
    y = "Error medio (votos)"
  ) +
  theme_minimal()
...
# Visualizar errores relativos (Opcional)
ggplot(media_errores_relativos, aes(x = partido, y = error_relativo_medio, fill = media)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Error relativo medio por medio y partido",
    x = "Partido",
    y = "Error relativo medio"
  ) +
  theme_minimal()

...
resultados_porcentajes <- election_data |> 
  mutate(
    Grupo = case_when(
      ...
      str_detect(siglas, "CS|UP|MP|VOX|CONVERGÈNCIA I UNIÓ") ~ "PARTIDO NUEVO",
      str_detect(siglas, "PNV|BNG|ERC|BILDU") ~ "NACIONALISTA/REGIONALISTA",
      TRUE ~ "OTROS"
    )
  ) |> 
...
plt1 <- ggplot(resultados_porcentajes, aes(fecha, porcentaje)) +
  geom_line(aes(color = fct_reorder(Grupo,porcentaje)), size = 2.4) +
  geom_point(
    aes(fill = Grupo), 
    size = 5, 
 ...
 scale_color_manual(values = c(BLUE, GREEN, BROWN,GREY)) +
  ...
  theme(
    ...
    axis.text = element_text(size = 12)                 # Estilo de texto de las marcas de los ejes
  ) +
  labs(
